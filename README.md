...
- **記録詳細**: 肉球アイコンをクリックして記録の詳細を表示
- **記録の編集・削除**: 保存済みの記録の編集と削除が可能
...
│       │   ├── auth/               # 認証関連コンポーネント
│       │   │   └── AuthProvider.tsx  # 認証プロバイダー（useAuthフックをラップ）
│       │   └── map/                # 地図関連コンポーネント
│       │       ├── Googlemap.tsx      # Google Mapメインコンポーネント
│       │       └── Googlemap.module.css
│       ├── hooks/                  # カスタムフック
│       │   ├── useAuth.tsx          # 認証フック（ログイン、サインアップ、ログアウト）
│       │   └── useRecords.tsx       # 記録データフック（保存、取得）
│       ├── lib/                    # ユーティリティ
...
### コンポーネント
- **Googlemap.tsx**: Google Map の表示、記録の作成・編集・削除、マーカー表示、ホバーポップアップ
- **AuthProvider.tsx**: 認証状態の管理と提供

### カスタムフック
- **useAuth.tsx**: Firebase Authentication による認証機能（サインイン、サインアップ、サインアウト、認証状態監視）
- **useRecords.tsx**: Firestore と Storage を使った記録の保存・取得

### ページ
- **page.tsx (root)**: ホームページ、地図の表示、使い方モーダル
- **login/page.tsx**: ログイン・サインアップページ、パスワードリセット
- **password-reset/page.tsx**: パスワード再設定ページ
- **records/page.tsx**: 記録一覧ページ、記録の編集・削除

### 設定
- **firebase.ts**: Firebase の初期化（Authentication、Firestore、Storage）
- **layout.tsx**: アプリケーション全体のレイアウト、フォント設定

## 使い方

### 初回利用

1. **アカウント作成**: トップページで「きろくする」→「アカウント作成」をクリック
2. **メールアドレスとパスワードを入力**して登録
...
- **ホバー表示**（PC）: 地図上の肉球アイコンにマウスをホバーすると、写真やコメントをポップアップ表示
- **詳細表示**: 肉球アイコンをクリックすると、記録の詳細をモーダル表示
- **記録一覧**: ヘッダーの「きろく一覧」から過去の記録を一覧表示

### 記録を編集・削除

1. **記録詳細を開く**: 肉球アイコンをクリック、または記録一覧から選択
2. **編集モード**: 「編集・削除」ボタンをクリック
3. **編集**: 画像やコメントを変更して「保存」
4. **削除**: 「削除」ボタンで記録を削除

### その他

- **現在地移動**: 左下のナビゲーションアイコンをクリックで現在地に移動
- **使い方**: ヘッダーの「つかいかた」で簡易説明を表示
- **ログアウト**: ヘッダーの「ログアウト」でログアウト
- **パスワードリセット**: ログイン画面で「パスワードの再設定」からメールで再設定リンクを受信
